...
int getFingerprintIDez() {
  ...
  Serial.print("Tanimli ID #"); Serial.println(finger.fingerID); 
  //anlamak için led yakıp söndürme
...
uint8_t getFingerprintEnroll() {
  ...
  int p = -1;
  Serial.print("ID'ye atanmak icin parmakizi bekleniyor"); Serial.println(id);
  ...
  // OK converted!
  Serial.print("#ID icin model olusturuluyor");  Serial.println(id);
  
  p = finger.createModel();
  if (p == FINGERPRINT_OK) {
    Serial.println("Eslendi");
  } else if (p == FINGERPRINT_PACKETRECIEVEERR) {
    Serial.println("Communication error");
    return p;
